...
// Ряд: [[iso, close, adjclose], ...]. close — для графиков (номинал),
// adjclose (сплиты+дивиденды) — для доходностей.
...
export async function fetchYahooDaily(ticker, fromIso) {
  ...
  for (let i = 0; i < r.timestamp.length; i++) {
    ...
    const row = [iso, rnd(Number.isFinite(c) && c > 0 ? c : a), rnd(a)];
...
export async function fetchStooqDaily(ticker, fromIso) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (c[0] && Number.isFinite(close) && close > 0) series.push([c[0], close, close]);
  }
  return series.length ? { series } : { missing: true };
}
...
export function readPriceCache(dataDir, ticker) {
  ...
  for (const line of text.split('\n')) {
    if (!line) continue;
    const c = line.split(',');
    series.push([c[0], Number(c[1]), Number(c[2] ?? c[1])]);
  }
  return series;
}
...
export function writePriceCache(dataDir, ticker, series) {
  ...
  writeFileSync(p, gzipSync(series.map(r => r[0] + ',' + r[1] + ',' + r[2]).join('\n'), { level: 9 }));
}
