...
// покупка при цене не ниже 7% от 52-недельного максимума, на существенную сумму, в бумаге
// с торгуемым оборотом; удержание три месяца. Пороги не на ножевом крае — по близости к
// максимуму эффект меняется плавно (−2% → +8.5%, −5% → +7.3%, −7% → +7.7%, −10% → +6.8%,
// −20% → +1.2%), по сумме плато от $10 тыс. до $150 тыс.
// Подробности и проверки — docs/ЧТО-РАБОТАЕТ.md.
//
// Порог близости был −5% и стал −7% (11.08.2026). Перебор 2240 комбинаций с выбором только
// по 2016–2020 и судом только по 2021–2026 (audit/h3–h5): −7% лучше −5% по всем показателям,
// кроме максимальной просадки (−31.0% против −30.1%), и даёт на четверть больше позиций
// (34 против 27) при МЕНЬШЕМ обороте. На скошенной выплате — медиана сделки −0.5% при
// среднем +2.7% — число позиций важнее долей процента: результат делают редкие крупные
// выигрыши, и на узком портфеле их легко не поймать. Дальше −10% эффект начинает
// разваливаться (t 1.67), −20% уже пустой.
const SET_MAX_DD = -0.07;
...
// медиана расхождения 1.4 п.п., 90-й процентиль 3.7 п.п., то есть при пороге такого порядка
// состав набора заметно смещался в сторону плательщиков дивидендов.
...
for (const [t, rows] of byTicker) {
  ...
  for (const r of rows) {
    ...
    // Цена, по которой подписчик реально входит: закрытие первого торгового дня после подачи.
    // Показывать рядом с «изменением» только цену инсайдера нельзя — при просроченной подаче
    // они расходятся до десятков процентов (GH: инсайдер купил по 129.82, вход по правилу
    // 170.77 через две недели), и колонка «изм.» выглядела бы посчитанной неверно.
    r._entryDate = e >= 0 ? s[e][0] : null;
    r._entryPx = e >= 0 ? rnd(s[e][1] * nominalFactor(sp, s[e][0]), 4) : null;
    ...
    // Вход пришёлся на последний бар ряда — измерять нечего: «0.0%» здесь не факт, а артефакт
    // нулевого срока, и в таблице он читался как «бумага стоит на месте».
    r._chg = e >= 0 && e < s.length - 1 ? rnd(lastAdj / s[e][2] - 1) : null;
...
const feedRows = buys.filter(r => r.fdate >= feedCut).map(r => ({
  ...
  // Дата выхода привязана к дню входа, но у сигнала, поданного сегодня, дня входа ещё нет.
  // Пустой «выход» у самой свежей — и самой пригодной для входа — позиции бесполезен,
  // поэтому откатываемся на подачу: разница максимум в торговый день на трёхмесячном сроке.
  exit: addDaysIso(r._fw?.entry ?? r.fdate, SET_HOLD_MONTHS * 30),
  ...
  entryPx: r._entryPx ?? null, entryDate: r._entryDate ?? null,
...
const SET_DEF = {
  ...
  ladders: [
    {
      // −5% оставлен в лестнице рядом с рабочим −7%: соседняя ступень показывает, что выбор
      // порога не подогнан и результат между ними меняется плавно.
      name: 'близость к 52-нед максимуму', vals: [-0.01, -0.02, -0.05, -0.07, -0.10, -0.20, -0.35],
...
W('stats.json', {
  built: today,
  // Дата последней котировки в снимке. Колонка «Сейчас» — это ЗАКРЫТИЕ этого дня, а не
  // текущая цена: сборка идёт раз в сутки после закрытия, и в течение торгового дня
  // снимок отстаёт на 15–20 часов, а после выходных — на трое суток. Без явной даты
  // колонка молча выдаёт себя за настоящее время.
  pricesAsOf: spy?.length ? spy[spy.length - 1][0] : null,
