...
export class MarketChart {
  constructor(canvas) {
    ...
    canvas.parentElement.appendChild(this.tip);
    canvas.addEventListener('mousemove', e => this.onMove(e));
    canvas.addEventListener('mouseleave', () => { this.tip.style.display = 'none'; this.hover = null; this.draw(); });
    new ResizeObserver(() => this.draw()).observe(canvas.parentElement);
  }

  set(weeks, thresholds) {
    this.weeks = weeks.filter(w => w.z !== null && w.spy !== null);
    this.thr = thresholds ?? { warn: 2, strong: 3 };
    this.hover = null;
    this.draw();
  }
  ...
  draw() {
    const ws = this.weeks;
    const dpr = window.devicePixelRatio || 1;
    const w = this.canvas.parentElement.clientWidth, h = 420;
    this.canvas.width = w * dpr; this.canvas.height = h * dpr;
    this.canvas.style.height = h + 'px';
    const ctx = this.ctx;
    ...
    const padL = 44, padR = 12, gap = 26;
    const topH = 210, botH = 120, topY = 12, botY = topY + topH + gap;
    const iw = w - padL - padR;
    const X = i => padL + (i + 0.5) / ws.length * iw;
    this.X = X; this.geom = { padL, iw, topY, topH, botY, botH, w, h };

    const zMax = Math.max(4, ...ws.map(x => x.z)), zMin = Math.min(-2, ...ws.map(x => x.z));
    const ZY = z => topY + (zMax - z) / (zMax - zMin) * topH;
    const grid = cssVar('--grid'), ink3 = cssVar('--ink-3');
    ctx.font = '11px system-ui, sans-serif';

    // Панель 1: нормированная кластерная активность
    ctx.strokeStyle = grid; ctx.lineWidth = 1;
    ctx.fillStyle = ink3; ctx.textAlign = 'right'; ctx.textBaseline = 'middle';
    for (const z of [-2, 0, 2, 4].filter(z => z >= zMin && z <= zMax)) {
      const y = ZY(z);
      ctx.beginPath(); ctx.moveTo(padL, y); ctx.lineTo(w - padR, y); ctx.stroke();
      ctx.fillText(z === 0 ? '0' : (z > 0 ? '+' : '') + z + 'σ', padL - 6, y);
    }
    // Пороги, на которых индикатор исторически работал
    ctx.setLineDash([5, 4]);
    for (const [t, label] of [[this.thr.warn, 'повышенная'], [this.thr.strong, 'вспышка']]) {
      if (t > zMax) continue;
      const y = ZY(t);
      ctx.strokeStyle = cssVar('--c-sell'); ctx.beginPath();
      ctx.moveTo(padL, y); ctx.lineTo(w - padR, y); ctx.stroke();
      ctx.fillStyle = cssVar('--c-sell'); ctx.textAlign = 'left';
      ctx.fillText(label, padL + 4, y - 8);
    }
    ctx.setLineDash([]);
    const bw = Math.max(1, iw / ws.length - 0.5);
    const zero = ZY(0);
    for (let i = 0; i < ws.length; i++) {
      const z = ws[i].z, y = ZY(z);
      ctx.fillStyle = z >= this.thr.strong ? cssVar('--c-sell')
        : z >= this.thr.warn ? cssVar('--c-buy')
        : z >= 0 ? 'color-mix(in srgb, ' + cssVar('--c-buy') + ' 45%, transparent)'
        : cssVar('--grid');
      const top = Math.min(y, zero), hh = Math.max(1.5, Math.abs(zero - y));
      ctx.fillRect(X(i) - bw / 2, top, bw, hh);
    }

    // Панель 2: SPY (лог-шкала — за 10 лет линейная сжимает начало истории)
    const sp = ws.map(x => Math.log(x.spy));
    const sMin = Math.min(...sp), sMax = Math.max(...sp);
    const SY = v => botY + (sMax - Math.log(v)) / (sMax - sMin || 1) * botH;
    ctx.strokeStyle = grid; ctx.beginPath();
    ctx.moveTo(padL, botY + botH); ctx.lineTo(w - padR, botY + botH); ctx.stroke();
    ctx.strokeStyle = cssVar('--c-line'); ctx.lineWidth = 2; ctx.lineJoin = 'round';
    ctx.beginPath();
    ws.forEach((x, i) => i ? ctx.lineTo(X(i), SY(x.spy)) : ctx.moveTo(X(i), SY(x.spy)));
    ctx.stroke();
    ctx.fillStyle = ink3; ctx.textAlign = 'left'; ctx.textBaseline = 'top';
    ctx.fillText('SPY (лог)', padL + 2, botY + 2);

    // Годы по общей оси
    ctx.textAlign = 'center';
    let lastYear = '';
    for (let i = 0; i < ws.length; i++) {
      const y = ws[i].w.slice(0, 4);
      ...
      ctx.setLineDash([]);
      ctx.fillStyle = ink3;
      ctx.fillText(y, x, botY + botH + 6);
    }

    if (this.hover !== null) {
      const x = X(this.hover);
      ctx.strokeStyle = ink3; ctx.setLineDash([3, 3]);
      ctx.beginPath(); ctx.moveTo(x, topY); ctx.lineTo(x, botY + botH); ctx.stroke();
      ctx.setLineDash([]);
    }
  }
  ...
  onMove(e) {
    ...
    const rect = this.canvas.getBoundingClientRect();
    const mx = e.clientX - rect.left, my = e.clientY - rect.top;
    const i = Math.max(0, Math.min(this.weeks.length - 1,
      Math.round((mx - this.geom.padL) / this.geom.iw * this.weeks.length - 0.5)));
    this.hover = i;
    this.draw();
    const x = this.weeks[i];
    this.tip.innerHTML = `<b>неделя ${esc(x.w)}</b><br>` +
      `эмитентов с кластером: <b>${x.ci}</b> (${x.z >= 0 ? '+' : ''}${x.z}σ)<br>` +
      `покупок: ${fmtInt(x.b)} · продаж: ${fmtInt(x.s)}<br>SPY: ${fmtPrice(x.spy)}`;
    this.tip.style.display = 'block';
    const tw = this.tip.offsetWidth;
    this.tip.style.left = Math.min(mx + 14, this.geom.w - tw - 6) + 'px';
    this.tip.style.top = Math.max(4, my - 50) + 'px';
  }
}
